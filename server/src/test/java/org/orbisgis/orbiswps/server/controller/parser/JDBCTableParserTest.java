...
import org.orbisgis.orbiswps.server.model.JDBCTable;
...
/**
 * Test class for the JDBCTableParser
 *
 * @author Sylvain PALOMINOS
 */
public class JDBCTableParserTest {

    /** JDBCTable parser. */
    private JDBCTableParser jdbcTableParser;
    /** Valid JDBCTable type list to use in test. */
    private List<DataType> validJDBCTableTypeList;
    /** Valid excluded type list to use in test. */
    private List<DataType> validExcludedTypeList;

    @Before
    public void initialization(){
        jdbcTableParser = new JDBCTableParser();

        validJDBCTableTypeList = new ArrayList<>();
        validJDBCTableTypeList.add(DataType.MULTILINESTRING);
        validJDBCTableTypeList.add(DataType.POLYGON);
    ...
    public void testAnnotation(){
        Assert.assertEquals("The JDBCTableParser annotation class should be 'JDBCTableAttribute'.",
                JDBCTableAttribute.class, jdbcTableParser.getAnnotation());
    }
    ...
    public void testSimplestParseInput(){
        Field field = null;
        try {
            field = JDBCTableParserTest.FieldProvider.class.getDeclaredField("simplestInput");
        ...
        try {
            inputDescriptionType = jdbcTableParser.parseInput(field, "tableName",
        ...
        Assert.assertTrue("The DataDescriptionType from the InputDescriptionType should be an instance of " +
                "JDBCTable.", dataDescriptionType instanceof JDBCTable);
        JDBCTable jdbcTable = (JDBCTable) dataDescriptionType;
        Assert.assertEquals("The JDBCTable defaultValues attribute should be 'tableName'.",
                "tableName", jdbcTable.getDefaultValue());
        Assert.assertTrue("The JDBCTable dataTypeList attribute should be empty.",
                jdbcTable.getDataTypeList().isEmpty());
        Assert.assertTrue("The JDBCTable excludedTypeList attribute should be empty.",
                jdbcTable.getExcludedTypeList().isEmpty());
    ...
    public void testComplexParseInput(){
        Field field = null;
        try {
            field = JDBCTableParserTest.FieldProvider.class.getDeclaredField("complexInput");
        } catch (NoSuchFieldException ignored) {}
        ...
        Assert.assertEquals("The JDBCTable defaultValues attribute should be 'tableName'.",
                "tableName", jdbcTable.getDefaultValue());
        Assert.assertTrue("The JDBCTable dataTypeList attribute should contains 'MULTILINESTRING', 'POLYGON'.",
                listEqualsNoOrder(jdbcTable.getDataTypeList(), validJDBCTableTypeList));
        Assert.assertTrue("The JDBCTable excludedTypeList attribute should contains 'POINT', 'MULTIPOINT'.",
                listEqualsNoOrder(jdbcTable.getExcludedTypeList(), validExcludedTypeList));
    ...
    public void testSimplestParseOutput(){
        Field field = null;
        try {
            field = JDBCTableParserTest.FieldProvider.class.getDeclaredField("simplestOutput");
        } catch (NoSuchFieldException ignored) {}
        ...
        String processId = "processid:"+UUID.randomUUID().toString();
        try {
            outputDescriptionType = jdbcTableParser.parseOutput(field, "tableName",
                    URI.create(processId));
        } catch (MalformedScriptException ignored) {}
    ...
    private class FieldProvider{
        ...
        private String simplestInput = "tableName";
